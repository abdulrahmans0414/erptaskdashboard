...
export const fetchGmailMails = async (userEmail, userRole, queryParams = {}) => {
    ...
    } else if (folderVal === 'FAILED') {
        gmailFolder = '[Gmail]/Trash';
    } else if (folderVal === 'ALL' || typeVal) {
    ...
    const paginatedUids = uidsArray.slice(startIndex, startIndex + limitVal * 3);
    ...
    if (paginatedUids.length > 0) {
        try {
            ...
            for (const message of fetchedMessages) {
                if (mails.length >= limitVal) break;
                ...
                if (message?.source) {
                    ...
                    if (typeVal && typeVal !== 'ALL' && mailType !== typeVal) {
                        continue;
                    }
                    ...
                    if (actualUserRole !== 'admin' && actualUserEmail) {
                        const emailLower = actualUserEmail.toLowerCase().trim();
    ...
    // IMAP ITERATION SAFEGUARD: Wrap returned mails array in validation check
    if (!mails || !Array.isArray(mails)) {
        console.log("No valid emails iterable array found");
        return [];
    }

    // If caller specified simple limit (like fetchGmailMails(20)), return array directly
    if (typeof userEmail === 'number') {
        return mails;
    }
    ...
    return {
        ...
        page: pageVal,
        pages: Math.ceil(uidsArray.length / limitVal),
        limit: limitVal
    };
};
