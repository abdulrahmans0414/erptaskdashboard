...
export default function Tasks() {
    ...
    useEffect(() => {
        ...
        setFilters(prev => {
            ...
            if (user.role === 'department-head' && user.department) next.department = user.department;
    ...
    const stats = useMemo(() => ({
        ...
        approved: allTasks.filter(t=>['approved','completed'].includes(t.status)).length,
    }), [allTasks]);
    ...
    const STAT_CARDS = [
        ...
        {l:"Submitted",v:stats.submitted,c:"text-violet-600",bg:"bg-violet-50"},
    ...
    const deptFilterOptions = (() => {
        ...
        if (user.role === 'department-head') return [user.department].filter(Boolean);
        ...
        <div className="max-w-6xl mx-auto space-y-5 p-4 md:p-6">
            ...
            <div className="flex items-start justify-between gap-3">
                ...
                <div className="flex items-center gap-2">
                    <button onClick={()=>load()} className="px-3 py-2 text-sm bg-gray-100 hover:bg-gray-200 rounded-xl transition font-medium text-gray-600">
                        🔄 Refresh
                    </button>
                    {canAssign && (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-2">
                {STAT_CARDS.map(s => (
                    <div key={s.l} className={`${s.bg} rounded-2xl p-3 text-center border border-gray-100`}>
                        <p className="text-[10px] text-gray-500 font-medium uppercase tracking-wide">{s.l}</p>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-2">
                    ...
                    {[
                        {k:'status', opts:["all","pending","in-progress","submitted","approved","rejected"]},
                        ...
                        <select key={k} value={filters[k]} onChange={e=>setF(k,e.target.value)}
                            disabled={(k==='branch' && !['admin','it'].includes(user?.role) && branchFilterOptions.length===1) || (k==='department' && user?.role==='department-head')}
                            ...
                            {opts.map(o=><option key={o} value={o} className="capitalize">{o==='all'?`All ${k}s`:o}</option>)}
                        </select>
                    ))}
                </div>
            </div>
            ...
            <div className="flex items-center justify-between text-xs text-gray-500">
                <span>Showing <strong>{current.length}</strong> of <strong>{filtered.length}</strong> tasks</span>
                {Object.values(filters).some(v=>v!=='all'&&v!=='') && (
                    <button onClick={()=>{setFilters({search:"",status:"all",priority:"all",department:"all",branch:"all"});setPage(1);}}
                        className="text-blue-600 hover:underline font-medium">Clear filters</button>
                )}
            </div>
            ...
            {loading ? (
                ...
                <div className="text-center py-16 bg-white rounded-2xl border border-gray-100">
                    ...
                    <p className="text-gray-400 text-sm mt-1">
                        {Object.values(filters).some(v=>v!=='all'&&v!=='') ? "Try clearing your filters" : "No tasks assigned yet"}
                    </p>
                </div>
            ) : (
                <div className="space-y-3">
                    {current.map(t => <TaskCard key={t._id} task={t} onUpdate={load} />)}
                </div>
            )}
            ...
            {totalPages > 1 && (
                <div className="flex items-center justify-center gap-1.5">
                    <button onClick={()=>setPage(p=>Math.max(1,p-1))} disabled={page===1}
                        className="px-3 py-1.5 rounded-xl text-sm bg-white border disabled:opacity-40 hover:bg-gray-50 transition">◀</button>
                    {Array.from({length:Math.min(totalPages,5)},(_,i)=>{
                        let p = totalPages<=5 ? i+1 : page<=3 ? i+1 : page>=totalPages-2 ? totalPages-4+i : page-2+i;
                        return <button key={p} onClick={()=>setPage(p)}
                            className={`w-9 h-9 rounded-xl text-sm transition ${page===p?"bg-blue-600 text-white font-bold shadow-sm":"bg-white border hover:bg-gray-50"}`}>{p}</button>;
                    })}
            ...
            <CreateTaskModal isOpen={showCreate} onClose={()=>setShowCreate(false)} onTaskCreated={load} />
        </div>
    );
}
